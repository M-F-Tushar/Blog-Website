...
const Tags: React.FC = () => {
      ...
      <div className="container mx-auto px-4 py-12 space-y-8">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {[...Array(10)].map((_, i) => (
            <div
              key={i}
              className="h-10 w-24 bg-secondary-100 dark:bg-secondary-800 rounded-full animate-pulse"
            />
          ))}
        </div>
      </div>
  ...
  if (error) {
    return <EmptyState icon="⚠️" title="Error Loading Tags" description={error} />;
  }

  return (
    <div className="space-y-12 pb-12">
      <SEO title="All Tags" description="Browse all tags used in the blog posts." />
      {/* Header */}
