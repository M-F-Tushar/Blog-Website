      ...
      <div className={`flex ${contentClasses[viewMode]}`}>
        <header className={viewMode === 'compact' ? 'flex-grow pr-12' : 'flex-grow'}>
          ...
          <div className="flex items-center flex-wrap gap-3 text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">
            ...
            <span className={`flex items-center ${viewMode !== 'compact' ? 'bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full' : ''}`}>
          ...
          <h2 id={`post-title-${post.id}`} className={`${viewMode === 'compact' ? 'text-lg' : 'text-xl'} font-bold font-serif mb-2 text-gray-900 dark:text-white line-clamp-2`}>
            <Link
              to={`/blog/${post.id}`}
