...
const ScrollProgress: React.FC<ScrollProgressProps> = ({ totalWords = 0 }) => {
  ...
  useEffect(() => {
    const unsubscribe = scrollYProgress.on('change', (latest) => {
      ...
      const currentMilestone = milestones.find((m) => percent >= m && m > lastMilestone);

      if (currentMilestone) {
        setLastMilestone(currentMilestone);
        setMilestoneText(
          currentMilestone === 100 ? '🎉 Article Complete!' : `${currentMilestone}% Complete`
        );
    ...
    <>
      ...
      {percentage > 0 && (
        <motion.div
          ...
          className="fixed top-4 right-4 z-[60] bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg border border-gray-200 dark:border-gray-700"
        >
          <div className="flex items-center gap-2 text-sm font-medium">
            <span className="text-accent dark:text-accent-light font-bold">{percentage}%</span>
            {totalWords > 0 && (
              <>
